...
export const MAX_PLAYER_LEVEL = 7;
export const XP_REQUIREMENTS = [40, 55, 70, 90, 115, 150] as const;
...
export function getXpRequirementForLevel(level: number): number | null {
  if (!Number.isInteger(level) || level < 1 || level >= MAX_PLAYER_LEVEL) {
    return null;
  }
